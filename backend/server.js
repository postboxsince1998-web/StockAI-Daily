...
async function startServer() {
  // Ensure database schema is initialized and verified BEFORE running queries
  if (db.initDb) {
    await db.initDb();
  }
  ...
  try {
    const count = await db.prepare('SELECT COUNT(*) as cnt FROM companies').get();
    if (!count || count.cnt === 0) {
      console.log('📦 Database empty on launch. Seeding initial data...');
      await seedDatabase();
    }
  } catch (e) {
    console.warn('⚠️ Seeding check notice:', e.message);
  }
