...
export async function runChangeDetection(symbol, priceRecord, prevPriceRecord, fundamentals, newsArticles = []) {
  const events = [];
  ...
  if (absChange >= 3.0) {
    ...
    await stmtEvent.run(symbol, 'PRICE_MOVEMENT', importance, desc, 'ChangeDetectionService');
  ...
  if (priceRecord.high && priceRecord.fifty_two_week_high && priceRecord.high >= priceRecord.fifty_two_week_high * 0.995) {
    ...
    await stmtEvent.run(symbol, '52W_HIGH', 'HIGH', desc, 'ChangeDetectionService');
    ...
    await stmtEvent.run(symbol, '52W_LOW', 'HIGH', desc, 'ChangeDetectionService');
  ...
  if (priceRecord.volume && prevPriceRecord && prevPriceRecord.volume) {
    if (priceRecord.volume > prevPriceRecord.volume * 1.8) {
      ...
      await stmtEvent.run(symbol, 'VOLUME_SPIKE', 'MEDIUM', desc, 'ChangeDetectionService');
  ...
  if (fundamentals) {
    if (fundamentals.profit_growth && fundamentals.profit_growth > 15) {
      ...
      await stmtEvent.run(symbol, 'PROFIT_GROWTH', 'HIGH', desc, 'ChangeDetectionService');
    ...
    if (fundamentals.debt_to_equity && fundamentals.debt_to_equity < 0.3) {
      ...
      await stmtEvent.run(symbol, 'LOW_DEBT', 'LOW', desc, 'ChangeDetectionService');
  ...
  for (const news of newsArticles) {
    if (news.category === 'RESULTS' || news.category === 'ACQUISITION' || news.category === 'DEBT' || news.category === 'ORDER') {
      ...
      await stmtEvent.run(symbol, 'NEWS_EVENT', 'MEDIUM', desc, 'ChangeDetectionService');
